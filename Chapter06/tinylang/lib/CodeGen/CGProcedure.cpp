...
llvm::Value *CGProcedure::readVariable(llvm::BasicBlock *BB, Decl *D,
  ...
  if (auto *V = llvm::dyn_cast<VariableDeclaration>(D)) {
    if (V->getEnclosingDecl() == Proc)
      ...
      if (!LoadVal)
        return Global;
      auto *Inst = Builder.CreateLoad(mapType(D), Global);
      CGM.decorateInst(Inst, V->getType());
      return Inst;
    } else
      llvm::report_fatal_error("Nested procedures not yet supported");
    ...
    if (FP->isVar()) {
      ...
      auto *Inst =
          Builder.CreateLoad(mapType(FP)->getPointerTo(), FormalParams[FP]);
      CGM.decorateInst(Inst, FP->getType());
      return Inst;
    } else
      return readLocalVariable(BB, D);
...
llvm::Value *CGProcedure::emitExpr(Expr *E) {
  if (auto *Infix = llvm::dyn_cast<InfixExpression>(E)) {
    ...
    if (auto *VarDecl = llvm::dyn_cast<VariableDeclaration>(Decl))
      TypeDecl = VarDecl->getType();
    else {
      assert(llvm::cast<FormalParameterDeclaration>(Decl) &&
             "Expecting a FormalParameterDeclaration here!");
      auto *FPDecl = llvm::cast<FormalParameterDeclaration>(Decl);
      TypeDecl = FPDecl->getType();
    }
...
void CGProcedure::emitStmt(AssignmentStatement *Stmt) {
  ...
  auto *DesDecl = Desig->getDecl();
  auto &Selectors = Desig->getSelectors();
  if (Selectors.empty())
    writeVariable(Curr, DesDecl, Val);
  else {
    llvm::SmallVector<llvm::Value *, 4> IdxList;
    // First index for GEP.
    IdxList.push_back(llvm::ConstantInt::get(CGM.Int32Ty, 0));
    auto *Base = readVariable(Curr, DesDecl, false);
    ...
    if (!IdxList.empty()) {
      if (Base->getType()->isPointerTy()) {
        Base =
            Builder.CreateInBoundsGEP(mapType(DesDecl, false), Base, IdxList);
        auto *Inst = Builder.CreateStore(Val, Base);
        TypeDeclaration *TypeDecl;
        if (auto *VarDecl = llvm::dyn_cast<VariableDeclaration>(DesDecl))
          TypeDecl = VarDecl->getType();
        else {
          assert(llvm::cast<FormalParameterDeclaration>(DesDecl) &&
                 "Expecting a FormalParameterDeclaration here!");
          auto *FPDecl = llvm::cast<FormalParameterDeclaration>(DesDecl);
          TypeDecl = FPDecl->getType();
        }
        CGM.decorateInst(Inst, TypeDecl);
      } else {
...
void CGProcedure::emitStmt(IfStatement *Stmt) {
  ...
  llvm::BasicBlock *ElseBB = HasElse ? createBasicBlock("else.body") : nullptr;
...
void CGProcedure::run(ProcedureDeclaration *Proc) {
  ...
  for (auto I = Fn->arg_begin(), E = Fn->arg_end(); I != E; ++I, ++Idx) {
    ...
    if (CGDebugInfo *Dbg = CGM.getDbgInfo())
      DIVariables[FP] = Dbg->emitParameterVariable(FP, Idx + 1, Arg, BB);
  }
