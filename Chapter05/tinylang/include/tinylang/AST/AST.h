...
public:
  ArrayTypeDeclaration(Decl *EnclosingDecL, SMLoc Loc, StringRef Name,
                       Expr *Nums, TypeDeclaration *Type)
...
protected:
  Selector(SelectorKind Kind, TypeDeclaration *Type) : Kind(Kind), Type(Type) {}
...
public:
  DereferenceSelector(TypeDeclaration *Type) : Selector(SK_Dereference, Type) {}
...
public:
  ...
  static bool classof(const Expr *E) { return E->getKind() == EK_Designator; }
};
