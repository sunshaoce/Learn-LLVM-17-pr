...
CGDebugInfo::CGDebugInfo(CGModule &CGM) : CGM(CGM), DBuilder(*CGM.getModule()) {
  llvm::SmallString<128> Path(CGM.getASTCtx().getFilename());
  llvm::sys::fs::make_absolute(Path);

  llvm::DIFile *File = DBuilder.createFile(llvm::sys::path::filename(Path),
  ...
  CU = DBuilder.createCompileUnit(
      llvm::dwarf::DW_LANG_Modula2, File, "tinylang", IsOptimzed, StringRef(),
      ObjCRunTimeVersion, StringRef(), EmissionKind);
}
...
llvm::DIType *CGDebugInfo::getPervasiveType(TypeDeclaration *Ty) {
  if (Ty->getName() == "INTEGER") {
    return DBuilder.createBasicType(Ty->getName(), 64,
                                    llvm::dwarf::DW_ATE_signed);
  }
  if (Ty->getName() == "BOOLEAN") {
    return DBuilder.createBasicType(Ty->getName(), 1,
...
llvm::DIType *CGDebugInfo::getAliasType(AliasTypeDeclaration *Ty) {
  return DBuilder.createTypedef(getType(Ty->getType()), Ty->getName(),
...
llvm::DIType *CGDebugInfo::getArrayType(ArrayTypeDeclaration *Ty) {
  ...
  return DBuilder.createArrayType(
      DL.getTypeSizeInBits(ATy) * 8, 1 << Log2(DL.getABITypeAlign(ATy)),
      getType(Ty->getType()), DBuilder.getOrCreateArray(Subscripts));
}

llvm::DIType *CGDebugInfo::getRecordType(RecordTypeDeclaration *Ty) {
...
llvm::DISubroutineType *CGDebugInfo::getType(ProcedureDeclaration *P) {
  ...
  for (const auto *FP : P->getFormalParams()) {
    ...
    if (FP->isVar()) {
      ...
      PT = DBuilder.createReferenceType(llvm::dwarf::DW_TAG_reference_type, PT,
...
void CGDebugInfo::emitProcedure(ProcedureDeclaration *Decl,
  ...
  llvm::DISubprogram *Sub = DBuilder.createFunction(
      getScope(), Decl->getName(), Fn->getName(), CU->getFile(),
      getLineNumber(Decl->getLocation()), SubT,
...
CGDebugInfo::emitParameterVariable(FormalParameterDeclaration *FP, size_t Idx,
  ...
  llvm::DILocalVariable *Var = DBuilder.createParameterVariable(
      ...
      getLineNumber(FP->getLocation()), getType(FP->getType()));
  DBuilder.insertDbgValueIntrinsic(Val, Var, DBuilder.createExpression(),
